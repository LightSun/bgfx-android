	   ...
	   &&  err.isOk() )
	{
		switch (chunk)
		{
			...
			case kChunkVertexBufferCompressed:
			{
				...
				//DBG("meshopt_decodeVertexBuffer need fix, compile");
			...
			case kChunkIndexBufferCompressed:
			{
                ...
                //DBG("meshopt_decodeIndexBuffer need fix compile");
...
void meshSubmit2(const Mesh* _mesh, const MeshState*const* _state, uint8_t _numPasses, const float* _mtx, uint16_t _numMatrices)
{
