...
namespace h7{
    ...
    static void postTouchEvent (AndroidInput* input, int type, int x, int y, int pointer, int button, long long timeStamp) {
        ...
        //synchronized (input) {
            switch (action) {
                ...
                case MotionEventWrapper::ACTION_POINTER_DOWN:
                    ...
                    if (button != -1) postTouchEvent(ainput, TouchEvent::TOUCH_DOWN, x, y, realPointerIndex, button, timeStamp);
                ...
                case MotionEventWrapper::ACTION_OUTSIDE:
                  ...
                  /*  x = (int)event.getX(pointerIndex);
                    ...
                    if (button != -1) postTouchEvent(ainput, TouchEvent::TOUCH_UP, x, y, realPointerIndex, button, timeStamp);
                    input.touchX[realPointerIndex] = x;
                    input.touchY[realPointerIndex] = y;
                    input.deltaX[realPointerIndex] = 0;
                    input.deltaY[realPointerIndex] = 0;
                ...
                case MotionEventWrapper::ACTION_MOVE:
                    ...
                    for (int i = 0; i < pointerCount; i++) {
                        ...
                        button = input.button[realPointerIndex];
                        if (button != -1)
                            postTouchEvent(ainput, TouchEvent::TOUCH_DRAGGED, x, y, realPointerIndex, button, timeStamp);
                        else
                            postTouchEvent(ainput, TouchEvent::TOUCH_MOVED, x, y, realPointerIndex, 0, timeStamp);
