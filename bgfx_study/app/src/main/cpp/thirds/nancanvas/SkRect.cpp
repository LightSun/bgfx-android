...
bool SkIRect::intersect(const SkIRect& r) {
    SkIRect tmp = {
        bx::max(fLeft,   r.fLeft),
        bx::max(fTop,    r.fTop),
        bx::min(fRight,  r.fRight),
        bx::min(fBottom, r.fBottom)
    };
    if (tmp.isEmpty()) {
        return false;
    }
...
bool SkRect::setBoundsCheck(const SkPoint pts[], int count) {
    ...
    if (all_finite) {
        this->setLTRB(bx::min(min[0], min[2]), bx::min(min[1], min[3]),
                      bx::max(max[0], max[2]), bx::max(max[1], max[3]));
    } else {
        this->setEmpty();
    }
    return all_finite;
