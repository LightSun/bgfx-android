...
struct SkIRect {
    ...
    */
    void offsetTo(int32_t newX, int32_t newY) {
        fRight = fRight + newX - fLeft;
        fBottom = fBottom + newY - fTop;
        fLeft = newX;
        fTop = newY;
    }
    ...
    */
    bool intersectNoEmptyCheck(const SkIRect &a, const SkIRect &b) {
        ...
        SkIRect r = {
                bx::max(a.fLeft, b.fLeft),
                bx::max(a.fTop, b.fTop),
                bx::min(a.fRight, b.fRight),
                bx::min(a.fBottom, b.fBottom)
        };
        if (r.isEmpty()) {
            return false;
        }
    ...
    */
    SkIRect makeSorted() const {
        return MakeLTRB(bx::min(fLeft, fRight), bx::min(fTop, fBottom),
                        bx::max(fLeft, fRight), bx::max(fTop, fBottom));
    }
